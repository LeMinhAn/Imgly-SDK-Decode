...
import com.camerafilter.ScriptC_alpha;
import com.camerafilter.ScriptC_translate_3d_lut;
...
public class LutColorFilter extends ImageFilter implements ImageFilter.FilterConfigIntensity {
    ...
    @NonNull
    private ScriptC_alpha getAlphaRenderScript(float intensity) {
        Scripts scripts = getScript();
        ScriptC_alpha alphaScript = scripts.alphaScript == null ? null : scripts.alphaScript;//.get();

        if (alphaScript == null ) {
            alphaScript = new ScriptC_alpha(rs);
    ...
    private Allocation getLutCube(@NonNull RenderScript rs, boolean forceRender) {
        ...
        ScriptC_translate_3d_lut script = new ScriptC_translate_3d_lut(rs);
    ...
    protected Bitmap renderIntensity(@NonNull Bitmap source, @NonNull Bitmap blend, float intensity) {
        ...
        if (intensity != 1) {
            ...
            ScriptC_alpha alphaBlendScript = getAlphaRenderScript(intensity);
